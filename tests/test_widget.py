import pytest
from src.widget import mask_account_card, parse_iso_date, get_date
...
def test_mask_account_card_correct_mask(data, expected):
    assert mask_account_card(data) == expected


# проверка на отсутствие цифр для маски
@pytest.mark.parametrize('data_without_numbers', [
    '',
    '    ',
    'no digits'
    'Счет',
...
def test_mask_account_card_no_digits_returns_original(data_without_numbers):
...
def test_mask_account_card_digits_in_middle_not_masked(data, expected):
    assert mask_account_card(data) == expected
...
def test_get_date_iso_to_dd_mm_yyyy(iso_string, expected):
...
def test_get_date_invalid_data(bad_iso):
